...
function main() {
  ...
  const filename = path.join(__dirname, "genFiles/data.csv");

  const output_file = path.join(__dirname, "genFiles/claimer.json");

  //file that has the user claim list
  const userclaimFile = path.join(__dirname, "genFiles/claimer2.json");

  //contract of items being sent out
  const airdropContract = "0xCAFd0a3053f00C17bF6014Ab610811E459BDC5Ec";
